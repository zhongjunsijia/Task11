...
该脚本使用简化的数据生成方式和固定的alpha值，确保能生成有意义的Lasso回归系数组合图表。
"""
...
def generate_simple_pollution_data():
    """
    生成简化的污染数据
    """
    print("生成简化的污染数据...")
...
def plot_combined_lasso_coefficients(all_coefficients, features, pollutants):
    ...
    # 调整y轴范围，确保能看到所有系数
    all_coefs = np.concatenate(list(all_coefficients.values()))
    y_min = min(all_coefs) * 1.1
    y_max = max(all_coefs) * 1.1
    ...
    filename = f'simple_combined_lasso_coefficients_{timestamp}.png'
    filepath = os.path.join(output_dir, filename)
    
    plt.savefig(filepath, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"简化的组合Lasso回归系数图已保存到: {filepath}")
    
    return filepath


def generate_simple_combined_lasso_plot():
    """
    生成简化的组合Lasso回归系数图
    """
    # 生成数据
    df = generate_simple_pollution_data()
    ...
    for pollutant in pollutants:
        ...
        # 训练模型（使用较小的alpha值）
        coefficients = train_lasso_model(X, y, alpha=0.01)
...
def main():
    ...
    """
    print("=" * 80)
    print("生成简化的组合Lasso回归系数图")
    print("=" * 80)
    
    try:
        # 生成简化的组合Lasso回归系数图
        plot_path = generate_simple_combined_lasso_plot()
        
        print("\n" + "=" * 80)
        print("任务完成！")
        print("=" * 80)
        
        print("生成的简化组合Lasso回归系数图：")
        print(f"- {plot_path}")
