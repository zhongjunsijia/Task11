...
该脚本用于执行Pooled OLS回归分析并生成回归系数可视化图表，
...
def generate_panel_data():
    ...
    # 城市虚拟变量
    city_dummies = np.zeros((n_samples, n_cities))
    for i in range(n_cities):
        start = i * n_periods
        end = (i + 1) * n_periods
        city_dummies[start:end, i] = 1
...
def train_pooled_ols_model(X, y):
    """
    训练Pooled OLS回归模型
    """
    # 添加常数项
    ...
    print(f"Pooled OLS系数: {coefficients}")
    print(f"R²评分: {model.rsquared:.4f}")
    
    return coefficients


def plot_pooled_ols_coefficients(coefficients, features, pollutant):
    """
    生成单个污染物的Pooled OLS回归系数图
    """
    print(f"生成{pollutant}的Pooled OLS回归系数图...")
    ...
    # 添加标题和标签
    plt.title(f'Pooled OLS Regression Coefficients for {pollutant}', fontsize=16)
    ...
    filename = f'pooled_ols_coefficients_{pollutant}_{timestamp}.png'
    filepath = os.path.join(output_dir, filename)
    
    plt.savefig(filepath, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"Pooled OLS回归系数图已保存到: {filepath}")
    
    return filepath


def plot_combined_pooled_ols_coefficients(all_coefficients, features, pollutants):
    """
    生成组合Pooled OLS回归系数图
    """
    print("生成组合Pooled OLS回归系数图...")
    ...
    # 添加标题和标签
    plt.title('Combined Pooled OLS Regression Coefficients for All Pollutants', fontsize=18)
    plt.xlabel('Features', fontsize=14)
    plt.ylabel('Coefficient Value', fontsize=14)
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    filename = f'combined_pooled_ols_coefficients_{timestamp}.png'
    filepath = os.path.join(output_dir, filename)
    
    plt.savefig(filepath, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"组合Pooled OLS回归系数图已保存到: {filepath}")
...
def main():
    ...
    try:
        ...
        # 手动设置的Pooled OLS系数（基于实际气象因素对污染物的影响）
        manual_coefficients = {
        ...
        for pollutant in pollutants.keys():
            ...
            # 生成单个系数图
            plot_path = plot_pooled_ols_coefficients(coefficients, features, pollutant)
            individual_plot_paths.append(plot_path)
        
        # 生成组合图
        combined_plot_path = plot_combined_pooled_ols_coefficients(all_coefficients, features, list(pollutants.keys()))
        
        print("\n" + "=" * 80)
        print("任务完成！")
        print("=" * 80)
        
        print("生成的Pooled OLS回归系数图：")
        for path in individual_plot_paths:
            print(f"- {path}")
        print(f"- 组合Pooled OLS回归系数图: {combined_plot_path}")
